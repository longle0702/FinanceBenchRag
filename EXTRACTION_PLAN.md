...
| `table_row_group_size` | `--table-row-group-size` | `5` (unique row labels; was `20`) | Large tables split into multiple chunks of this many rows each. **Retrieval finding** (see `README.md` → "Problems"): with the old default of 20, a specific numeric fact (e.g. "Total assets") got buried in the embedding of ~20 unrelated line items and ranked #106/396 for a direct query about it; dropping this to `3` moved it to #5/471 in the same test. The default was changed to `5` as a middle ground (fewer/larger chunks than `3`, still far more targeted than `20`) — `5` itself was not independently re-tested, so treat it as a starting point for a real ablation (3 vs 5 vs 8...), not a validated optimum. |
